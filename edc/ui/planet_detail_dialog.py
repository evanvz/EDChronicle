...
def _stat_label(text):
    lbl = QLabel(text)
    lbl.setStyleSheet("color: #9aa4b0; font-size:12px; background: transparent; border: none;")
    return lbl
...
class PlanetDetailDialog(QDialog):
    ...
    def __init__(self, body_name: str, rec: dict, state, parent=None):
        ...
        if atmo_comp_str:
            ac_lbl = QLabel(
                f'<span style="color:#9aa4b0;font-size:12px;">ATMOSPHERE COMPOSITION&nbsp;&nbsp;</span>'
        ...
        if any_sig:
            ...
            if other_sig:
                sig_parts.append(
                    ...
                    f' <span style="color:#9aa4b0;font-size:12px;">'
        ...
        if est_i > 0:
            val_lbl = QLabel(
                f'<span style="color:#9aa4b0;font-size:12px;">ESTIMATED VALUE&nbsp;&nbsp;</span>'
