...
- Combat > System Status: War/CivilWar conflicts, multi-state factions (e.g. War+Outbreak), and RES/Low RES/High RES/Hazardous RES presence for every tracked system within a configurable radius — sourced from the player's own journal, a live EDDN subscription, and (War/CivilWar + multi-state factions only) EDSM as an always-on catch-up layer for whatever happened while the app wasn't running, piggybacked on the Player Faction tab's existing per-system EDSM refresh at no extra request cost. Same radius-search shape as the Market tab. RES signal detection is system-level only (no ring/body granularity), and has no EDSM fallback (EDSM carries no signal data at all). EDSM-sourced conflicts have no win-day score (EDSM's factions endpoint only reports each faction's own state, never a Conflicts/WonDays breakdown) — shown as "?" until a personal visit or EDDN sighting confirms the real score. Status reflects the most recently confirmed sighting, not full history, and stale results are excluded from search rather than shown as possibly-current — War/CivilWar/faction status past 7 days (wars resolve on a fixed 7-day cycle, so anything older is guaranteed over) and RES presence past 14 days.
